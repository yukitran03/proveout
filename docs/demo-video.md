...
Every hash, address and figure you need is in [`FACTS.md`](FACTS.md), which is generated by
`npm run finalize`. Read them from there, never from memory.

Target: **3 minutes.** Judges watch a lot of these. A tight three minutes that proves one
...
# 2. The four demo transactions already exist and are in FACTS.md. Narrate over
#    those rather than re-running e2e: a fresh run costs two attestation waits.

# 3. Have the test suite ready to run on camera, it takes about a second
npm run test    # 97 passing
```

Open these tabs, in this order, and leave them open:

1. The live console, at the URL in FACTS.md
2. The **verify page**, `/verify` on the same site. This is your live moment.
3. Blockscout, the **challenge** transaction from FACTS.md row 2
4. Etherscan, the **self-certification refusal** from FACTS.md row 4
5. Blockscout, the **blocked replay** from FACTS.md row 3
6. `contracts/src/WorkOracle.sol` in your editor, on the reporter modifier
7. A terminal, in the repo, font size **16pt or larger**
...
| 2 | 0:18-0:42 | Title card or deck slide 3 | The insight |
| 3 | 0:42-1:02 | Live console | Show it exists and settles |
| 4 | 1:02-1:38 | Blockscout, challenge tx | **The money shot** |
| 5 | 1:38-2:05 | Etherscan, the refusal + WorkOracle.sol | The hole we found in our own design |
| 6 | 2:05-2:32 | The verify page, live | Let them check it themselves |
| 7 | 2:32-2:48 | Blockscout, reverted replay | Replay protection |
| 8 | 2:48-3:00 | Console or title card | Honest limit, close |
...
>
> Ninety-seven tests. The one named `test_release_isRefused_whenReceiptStatusIsZero` is that
> attack, and it's the most important test in the repo.

### Scene 5b — 1:38 to 2:05 · the hole we found in our own design

> [switch to Etherscan, the self-certification refusal]
>
> Here's one I'd rather show you than have you find.
>
> The first version of our source oracle had no access control. Anyone could call it. Which
> meant the builder could declare their own work complete, prove that honestly, and get paid
> for doing nothing. Every gate in the escrow passed, because they all check *which contract*
> emitted the event, and none of them can check who was allowed to call it.
>
> [beat]
>
> This transaction is the builder trying exactly that, after the fix. Reverted.
> `NotReporter`, and it names their address.
>
> [switch to WorkOracle.sol, on the modifier]
>
> Good news is cheap to fake, so it's gated. Carrying the proof to Creditcoin is still open to
> anyone, which is where being permissionless actually buys you something.

### Scene 6 — 2:05 to 2:32 · let them check it themselves

> [switch to the verify page]
>
> Last thing, and this is the part I'd want if I were reviewing this.
>
> [paste a Sepolia hash, hit verify, let it run on camera]
>
> That just called the real Block Prover precompile on Creditcoin, with a real proof, and came
> back verified. No wallet, nothing spent, because `verify` is a view function.
>
> And look at these as two separate answers: the precompile says the transaction was in the
> block, and the receipt says whether it actually succeeded. Conflating those two is the
> mistake I was talking about earlier.
>
> You can paste any Sepolia hash in here yourself.

### Scene 7 — 2:32 to 2:48
...
- [ ] The verify page actually ran on camera, and you did not cut the wait.
- [ ] You said the oracle is reporter-gated rather than implying nothing is trusted.
